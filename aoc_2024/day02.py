...
def part_1(raw: str) -> int:
    ...
    for report in reports:
        all_decreasing = all(n < 0 for n in report.diffs())
        all_increasing = all(n > 0 for n in report.diffs())

        in_range = all(abs(n) in [1, 2, 3] for n in report.diffs())
        count += in_range and (all_increasing or all_decreasing)
